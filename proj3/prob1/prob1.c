...
int main(int argc, char* argv[]){
    if (argc != 3){
        printf("Usage: %s scheduling_type# #_of_thread\n", argv[0]);
        return -1;
    }
    ...
    switch(scheduling_type_num){
        ...
        case 2:
            ...
            break;
        case 3:
            #pragma omp parallel for reduction(+:sum) private(x) schedule(static, 10)
            for(i=0; i<num_steps; i++){
                x = (i+0.5)*step;
                sum = sum + 4.0/(1.0+x*x);
            }
            break;
        case 4:
            #pragma omp parallel for reduction(+:sum) private(x) schedule(dynamic, 10)
            for(i=0; i<num_steps; i++){
                x = (i+0.5)*step;
                sum = sum + 4.0/(1.0+x*x);
            }
